...
function updateAdminUI() {
  ...
  if (badge)     badge.style.display     = loggedIn ? 'inline-flex' : 'none';
  if (panel && !loggedIn) panel.style.display = 'none';
}
...
function openAdminPanel() {
  if (!isAdminLoggedIn()) { showAdminLoginModal(); return; }
  const panel = document.getElementById('admin-panel');
  if (!panel) return;
...
function renderAdminPanel() {
  ...
  container.innerHTML = `
    <div class="admin-tabs">
      <button class="admin-tab active" data-tab="menu">🍽️ Menu</button>
      <button class="admin-tab" data-tab="tables">🪑 Tables</button>
      <button class="admin-tab" data-tab="reservations">📅 Reservations</button>
...
function boot() {
  initNavbarScroll();
  // BUG FIX: Don't run reveal observer immediately on boot.
  // Loading screen covers the hero for ~2s. If observer runs now, it marks
  // elements 'visible' and the CSS transition fires while hidden — so user
  // sees nothing when loading screen hides. The loading screen script in HTML
  // handles hero reveals. Run observer only for below-fold elements after delay.
  setTimeout(function() {
    observeRevealElements(null); // now safe — loading screen is gone
  }, 2200);
  initLazyImages(null);
